...
export function RouteSearchResults({
  ...
  const formatDuration = (timeInWay?: string) => {
    if (!timeInWay) return '';
    // Format HH:MM to "Xh Ym"
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {routes.map((route, index) => (
        <Card key={route.interval_id || index} className="hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="space-y-3 flex-1">
                ...
                <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                  ...
                  {route.free_seats !== undefined && route.free_seats.length > 0 && (
                    <div className="flex items-center space-x-1">
                      <Users className="h-4 w-4" />
                      <span>
                        {route.free_seats.length} {route.free_seats.length === 1 ? 'loc' : 'locuri'} libere
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="text-right space-y-2 ml-6">
                ...
                <Button 
                  ...
                  className="w-full"
                  disabled={!route.free_seats || route.free_seats.length === 0}
                >
                  {(!route.free_seats || route.free_seats.length === 0) ? 'Complet ocupat' : 'Selectează'}
                </Button>
              </div>
            </div>
          </CardContent>
